...
ggplot(timezone_map, aes(x = longitude, y = latitude, fill = place_name)) +
  ...
  theme(panel.grid.major = element_line(colour = "gray", size = 0.5), # Set plot theme
        ...
        axis.title = element_text(size = 12, face = "bold"), #axis parameters
        plot.title = element_text(size = 16, face = "bold"), #plot parameters
        panel.background = element_rect(fill = "white", colour = "green"),#border
